# libtabnasgbnf — the gbnf parser as a C ABI

<!-- tabnas-clib-template: v5 — stamped by admin tasks/adopt-clib.sh;
     edit the template and re-stamp, not this file. -->

The gbnf format parser as a C shared library, so languages with no
tabnas port can parse and validate gbnf input. This is one of the
...
| Function | Returns |
|---|---|
| `tabnas_version()` | `{"ok":true,"lib":"libtabnasgbnf","format":"gbnf","template":"v5"}` |
...
// link against libtabnasgbnf; every call returns a JSON []u8 to free with
// c.tabnas_free.
```

## Format notes

The input is GBNF grammar TEXT (llama.cpp notation); `accept:true` means it parses AND compiles (a `root` rule, every reference defined, no tokenizer-token terminals, no purely left-recursive rule). `value` is the grammar's compiled RECOGNITION SPEC: pure data that `libtabnas` (tabnas/parser) loads with `tabnas_grammar_json` to validate text against the grammar with no GBNF front-end present - regex tokens travel as `@~/.../` sentinel strings, so an ordinary JSON re-encode is safe. This replaces the former `gbnf_*` surface: `gbnf_compile` is this `tabnas_parse`, and `gbnf_grammar` + `gbnf_parse` become `libtabnas`'s `tabnas_grammar_json` + `tabnas_parse` over that `value`.
