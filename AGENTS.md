...
| [`go/clib/`](go/clib/) | `libgbnf`, the C ABI: GBNF text in, accept/reject verdict out. Compiles NATIVELY rather than via a serialized spec — a GBNF grammar's lexing settings are part of its language, so an in-process install is the form that keeps them. See [`go/clib/README.md`](go/clib/README.md). |
| [`py/`](py/) | The Python binding — `ctypes` over `libgbnf`. Reimplements nothing; graded against the same corpus and samples as the Go and TS suites. |
